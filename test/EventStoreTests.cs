...
public class EventStoreTests
{
    ...
    {
        using var table = new Wrapper();
        var streams = table.Sut.ListStreams(StreamType1).ToArray();
    ...
    public async Task CanCreateStream()
    {
        ...
        await table.Sut.CreateStream(streamId, StreamType1, CancellationToken.None);
        var stream = table.StreamTable.Query<EventStreamRecord>()
    ...
    public async Task CanTryCreateStream()
    {
        using var table = new Wrapper();
        var streamId = Guid.NewGuid().ToString("N");
        await table.Sut.CreateStreamIfNotExist(streamId, StreamType1, CancellationToken.None);
        var stream = table.StreamTable.Query<EventStreamRecord>()
            .Single(stream => stream.PartitionKey == streamId && stream.RowKey == streamId);
    ...
    {
        using var table = new Wrapper();
        await table.Sut.ArchiveStream(StreamId1, CancellationToken.None);
        var stream = table.StreamTable.GetEntity<EventStreamRecord>(StreamId1, StreamId1).Value;
    ...
    public async Task CanWriteStream()
    {
        ...
        await table.Sut.WriteStream(StreamId1, evt, UserId, CancellationToken.None);
        var stream = table.StreamTable.GetEntity<EventStreamRecord>(StreamId1, StreamId1).Value;
        stream.LatestVersion.Should().Be(4);

        var events = table.EventTable.Query<EventRecord>(e => e.PartitionKey == StreamId1);
    ...
    {
        using var table = new Wrapper();
        await Assert.ThrowsAsync<NotFoundException>(async () => await table.Sut.WriteStream(Guid.NewGuid().ToString("N"), new TestAEvent("a"), UserId, CancellationToken.None));
    }
    ...
    public async Task CanFailOptimisticStreamWrite()
    {
        ...
        await table.EventTable.AddEntityAsync<EventRecord>(new(StreamId1, 4, string.Empty, string.Empty, UserId));
        await Assert.ThrowsAsync<OptimisticWriteInterruptedException>(async () => await table.Sut.WriteStream(StreamId1, new TestAEvent(), UserId, CancellationToken.None));
    }
    ...
    {
        using var table = new Wrapper();
        var events = table.Sut.ReadStream(StreamId1).ToList();
    ...
    {
        using var table = new Wrapper();
        var events = table.Sut.ReadStream(StreamId1, 3).ToList();
    ...
    private class Wrapper : IDisposable
    {
        ...
        public Wrapper()
        {
            Postfix = Guid.NewGuid().ToString("N");
